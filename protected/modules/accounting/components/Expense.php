...
class Expense extends CComponent {
    ...
    public function flush() {
        JournalAccounting::model()->deleteAllByAttributes(array(
            'transaction_number' => $this->header->getCodeNumber(ExpenseHeader::CN_CONSTANT),
        ...
        foreach ($this->details as $detail) {
            ...
            if ((int)$detail->is_inactive === 0) {
                $accountingJournalDebit = AccountingJournalHelper::make(
                    'debit', 
                    $this->header->getCodeNumber(ExpenseHeader::CN_CONSTANT), 
                    AccountingJournalHelper::EXPENSE, 
                    $detail->account_id, 
                    $detail->amount, 
