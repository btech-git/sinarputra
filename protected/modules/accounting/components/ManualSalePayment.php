...
class ManualSalePayment extends CComponent {
    ...
    public function generateCodeNumber($currentMonth, $currentYear) {
        $salePaymentHeader = ManualSalePaymentHeader::model()->find(array(
    ...
    public function addInvoice($saleInvoiceHeaderId) {
        ...
        $exist = FALSE;
        $saleInvoiceHeader = ManualSaleInvoiceHeader::model()->findByPk($saleInvoiceHeaderId);

        if ($saleInvoiceHeader != null) {
            foreach ($this->details as $detail) {
                if ($detail->manual_sale_invoice_header_id == $saleInvoiceHeader->id) {
                    $exist = TRUE;
                    break;
                }
            }
            ...
            if (!$exist) {
                ...
                $detail->manual_sale_invoice_header_id = $saleInvoiceHeaderId;
    ...
    public function validateDetailsUnique() {
        ...
        for ($i = 0; $i < $detailsCount; $i++) {
            for ($j = $i; $j < $detailsCount; $j++) {
                ...
                if ($this->details[$i]->manual_sale_invoice_header_id === $this->details[$j]->manual_sale_invoice_header_id) {
                    $valid = false;
    ...
    public function flush() {
        JournalAccounting::model()->deleteAllByAttributes(array(
            'transaction_number' => $this->header->getCodeNumber(ManualSalePaymentHeader::CN_CONSTANT),
            'transaction_type' => AccountingJournalHelper::SALE_PAYMENT_MANUAL,
        ));
        ...
        foreach ($this->details as $detail) {
            ...
            if ($detail->isNewRecord) {
                $detail->manual_sale_payment_header_id = $this->header->id;
            }

            $valid = $detail->save(false) && $valid;

            $saleInvoiceHeader = ManualSaleInvoiceHeader::model()->findByPk($detail->manual_sale_invoice_header_id);
            ...
            $accountingJournalDebitAccount = AccountingJournalHelper::make(
                'debit', 
                $this->header->getCodeNumber(ManualSalePaymentHeader::CN_CONSTANT), 
                AccountingJournalHelper::SALE_PAYMENT_MANUAL, 
                $detail->account_id, 
                $detail->amount, 
            ...
            $receivableLedger->transaction_number = $this->header->getCodeNumber(ManualSalePaymentHeader::CN_CONSTANT);
            $receivableLedger->transaction_date = $this->header->date; 
            $receivableLedger->note = $detail->manualSaleInvoiceHeader->getCodeNumber(ManualSaleInvoiceHeader::CN_CONSTANT);
    ...
    public function getTotalReceivable() {
        ...
        foreach ($this->details as $detail) {
            $total += $detail->saleInvoiceGrandTotal;
        }
    ...
    public function getTotalRemaining() {
        return $this->totalReceivable - $this->totalPayment;
    }
