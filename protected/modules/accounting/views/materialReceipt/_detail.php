<table style="border: 1px solid">
    ...
    <?php foreach ($materialReceipt->details as $i => $detail): ?>
        <?php $tabIndex = $i * 2; ?>
        <?php $detailMaterialInvoice = $detail->materialInvoiceHeader; ?>

        <tr style="background-color: azure">
            <td style="text-align: center">
                <?php echo CHtml::activeHiddenField($detail, "[$i]material_invoice_header_id"); ?>
                <?php echo CHtml::encode($detailMaterialInvoice->getCodeNumber(MaterialInvoiceHeader::CN_CONSTANT)); ?>
            </td>
            <td style="text-align: center">
                <?php echo CHtml::encode(Yii::app()->dateFormatter->format('d MMMM yyyy', strtotime(CHtml::value($detailMaterialInvoice, 'date')))); ?>
            </td>
                ...
                <?php if ($detail->isNewRecord): ?>
                    <?php echo CHtml::button('Delete', array(
                        'onclick' => CHtml::ajax(array(
                            'type' => 'POST',
                            'url' => CController::createUrl('ajaxHtmlRemoveDetail', array('id' => $materialReceipt->header->id, 'index' => $i)),
                            'update' => '#detail_div',
                        )),
                    )); ?>
                <?php else: ?>
    ...
    <tr style="background-color: aquamarine">
        ...
        <td style="font-weight: bold; text-align: right">
            <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', CHtml::value($materialReceipt, 'totalReceipt'))); ?>
        </td>
        <td colspan="2"></td>
    </tr>
</table>
