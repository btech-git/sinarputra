...
<div style="font-weight: bold; text-align: center">
    ...
    <div style="font-size: larger">Laporan Pelunasan Manual 2</div>
...
<table class="report">
    ...
    <?php foreach ($materialPaymentSummary->dataProvider->data as $header): ?>
        <?php foreach ($header->materialPaymentDetails as $detail): ?>
            <tr>
                <td class="width1-1"><?php echo CHtml::encode($header->getCodeNumber(MaterialPaymentHeader::CN_CONSTANT)); ?></td>
                <td class="width1-2"><?php echo CHtml::encode(Yii::app()->dateFormatter->format('d MMM yyyy', strtotime($header->date_transaction))); ?></td>
                <td class="width1-3"><?php echo CHtml::encode(CHtml::value($header, 'customer.company')); ?></td>
                <td class="width1-4"><?php echo CHtml::encode($detail->materialInvoiceHeader->getCodeNumber(MaterialInvoiceHeader::CN_CONSTANT));?></td>
