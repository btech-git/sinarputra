...
<div style="font-weight: bold; text-align: center">
    ...
    <div style="font-size: larger">Laporan Outstanding Per Customer Detail</div>
...
<table class="report">
    ...
    <?php foreach ($saleReceiptSummary->dataProvider->data as $header): ?>
        <?php $dueDays = empty($header->saleReceiptDetails) ? 0 : strtotime($header->saleReceiptDetails[0]->saleReceiptHeader->due_date); ?>
        ...
        <?php endif; ?>
            <?php $saleReceiptDetail = empty($header->saleReceiptDetails) ? 0 : $header->saleReceiptDetails[0]; ?>
            <td class="width1-1"><?php echo CHtml::encode(Yii::app()->dateFormatter->format('d MMM yyyy', strtotime($header->date))); ?></td>
            <td class="width1-2">
                <?php echo empty($saleReceiptDetail) ? "N/A" : CHtml::encode(Yii::app()->dateFormatter->format('d MMM yyyy', strtotime($saleReceiptDetail->saleReceiptHeader->date))); ?>
            </td>
            <td class="width1-3"><?php echo CHtml::encode(CHtml::value($header, 'customer.invoice_due_days')); ?></td>
            <td class="width1-4">
                <?php echo empty($saleReceiptDetail) ? "N/A" : CHtml::encode(Yii::app()->dateFormatter->format('d MMM yyyy', strtotime($saleReceiptDetail->saleReceiptHeader->due_date))); ?>
            </td>
            <td class="width1-5"><?php echo $remainingDueDays; ?></td>
            <td class="width2-6"><?php echo CHtml::encode($header->getCodeNumber(SaleInvoiceHeader::CN_CONSTANT)); ?></td>
            <td class="width2-7"><?php echo CHtml::encode(CHtml::value($header, 'customer.company')); ?></td>
            <td class="width1-8"><?php echo CHtml::encode(CHtml::value($header, 'employeeIdSalesman.name')); ?></td>
