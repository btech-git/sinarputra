...
class MaterialReceiptController extends Controller {
    ...
    public function actionSummary() {
        ...
        $saleReceiptDetail = Search::bind(new MaterialReceiptDetail('search'), isset($_GET['MaterialReceiptDetail']) ? $_GET['MaterialReceiptDetail'] : array());
        ...
        $saleReceiptDetailSummary = new MaterialReceiptDetailSummary($saleReceiptDetail->search());
    ...
    protected function saveToExcel($saleReceiptDetailSummary, $startDate, $endDate) {
        ...
        $documentProperties->setTitle('Laporan TT Penjualan Material');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Laporan TT Penjualan Material');
        ...
        $worksheet->setCellValue('A2', 'Laporan Tanda Terima Penjualan Material');
        ...
        foreach ($saleReceiptDetailSummary->dataProvider->data as $header) {
            ...
            $number++;
            $saleReceiptHeader = $header->materialReceiptHeader;
            $saleInvoiceHeader = $header->materialInvoiceHeader;
            $worksheet->setCellValue("B{$counter}", $saleReceiptHeader->getCodeNumber(MaterialReceiptHeader::CN_CONSTANT));
            ...
            $worksheet->setCellValue("H{$counter}", $saleInvoiceHeader->getCodeNumber(MaterialInvoiceHeader::CN_CONSTANT));
        ...
        header('Content-Disposition: attachment;filename="Laporan Tanda Terima Penjualan Manual 2.xls"');
