...
//            'customerId' => $customerId,
    ...
    protected function saveToExcel($saleInvoiceSummary, $startDate, $endDate) {
        ...
        foreach ($saleInvoiceSummary->dataProvider->data as $header) {
            ...
            $worksheet->setCellValue("I{$counter}", $header->workOrderCuttingHeader->saleHeader->customer_order_number);
            ...
            $worksheet->setCellValue("N{$counter}", empty($header->saleReceiptDetails) ? "" : CHtml::encode($header->saleReceiptDetails[0]->saleReceiptHeader->date_receipt));
