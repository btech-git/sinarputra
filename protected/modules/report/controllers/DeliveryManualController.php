...
class DeliveryManualController extends Controller {
    ...
    protected function saveToExcel($deliveryManualSummary, $startDate, $endDate) {
        ...
        $worksheet->setCellValue('F5', 'Sopir');
        $worksheet->setCellValue('G5', 'Pembuat');
        $worksheet->setCellValue('H5', 'Catatan');
        $worksheet->setCellValue('I5', 'GRADE');
        $worksheet->setCellValue('J5', 'Kategori');
        $worksheet->setCellValue('K5', 'Tbl/Dmtr');
        $worksheet->setCellValue('L5', 'Lbr/Dmtr');
        $worksheet->setCellValue('M5', 'Panjang');
        $worksheet->setCellValue('N5', 'Berat');
        $worksheet->setCellValue('O5', 'Quantity');
        $worksheet->setCellValue('P5', 'M');
        $worksheet->setCellValue('Q5', 'SM');
        $worksheet->setCellValue('R5', 'G');
        $worksheet->setCellValue('S5', 'HT');
        $worksheet->setCellValue('T5', 'NTD');
        ...
        foreach ($deliveryManualSummary->dataProvider->data as $header) {
            foreach ($header->manualDeliveryDetails as $detail) {
                ...
                $worksheet->setCellValue("D{$counter}", CHtml::value($header, 'workOrderCuttingHeader.saleHeader.customer.company'));
                $worksheet->setCellValue("E{$counter}", CHtml::value($header, 'warehouse.name'));
                $worksheet->setCellValue("F{$counter}", CHtml::value($header, 'driver'));
                $worksheet->setCellValue("G{$counter}", CHtml::value($header, 'admin.username'));
                $worksheet->setCellValue("H{$counter}", CHtml::value($header, 'note'));
                $worksheet->setCellValue("I{$counter}", CHtml::value($detail, 'grade_name'));
                $worksheet->setCellValue("J{$counter}", CHtml::value($detail, 'productCategory.name'));
                $worksheet->setCellValue("K{$counter}", CHtml::value($detail, 'height'));
                $worksheet->setCellValue("L{$counter}", CHtml::value($detail, 'width'));
                $worksheet->setCellValue("M{$counter}", CHtml::value($detail, 'length'));
                $worksheet->setCellValue("N{$counter}", CHtml::value($detail, 'weight'));
                $worksheet->setCellValue("O{$counter}", CHtml::value($detail, 'quantity'));
                $worksheet->setCellValue("P{$counter}", $detail->is_miling == 1 ? "Yes" : "");
                $worksheet->setCellValue("Q{$counter}", $detail->is_sidemiling == 1 ? "Yes" : "");
                $worksheet->setCellValue("R{$counter}", $detail->is_grinding == 1 ? "Yes" : "");
                $worksheet->setCellValue("S{$counter}", $detail->is_hardness == 1 ? "Yes" : "");
                $worksheet->setCellValue("T{$counter}", $detail->is_annelying == 1 ? "Yes" : "");
