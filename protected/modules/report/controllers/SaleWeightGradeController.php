...
class SaleWeightGradeController extends Controller {
    ...
    public function actionSummary() {
        ...
        $sql = SqlViewGenerator::materialMonthlySalesWeight();
        ...
        foreach ($resultSet as $item) {
            if ($lastId != $item['grade_name']) {
                $data[$item['grade_name']]['grade_name'] = $item['grade_name'];
                $data[$item['grade_name']]['values'] = $records;
            }
            $month = intval($item['month']);
            $year = intval($item['year']);
            $yearMonth = $year . '-' . str_pad($month, 2, '0', STR_PAD_LEFT);
            if (isset($data[$item['grade_name']]['values'][$yearMonth])) {
                $data[$item['grade_name']]['values'][$yearMonth] = doubleval($item['grand_total']);
            }
            $lastId = $item['grade_name'];
        }
    ...
    protected function saveToExcel($data, $records) {
        ...
        $documentProperties->setTitle('Laporan Penjualan per Grade');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Laporan Penjualan per Grade');
        ...
        $worksheet->setCellValue('A2', 'Laporan Penjualan per Grade');
        ...
        $worksheet->setCellValue('A5', 'Grade Name');
        ...
        foreach ($data as $customerId => $item)
        {
            ...
            $worksheet->setCellValue("A{$counter}", CHtml::encode($item['grade_name']));
        ...
        header('Content-Disposition: attachment;filename="Laporan Omzet Per Grade.xls"');
