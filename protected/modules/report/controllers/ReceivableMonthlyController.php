...
class ReceivableMonthlyController extends Controller {
    ...
    public function actionSummary() {
        ...
        $sql = SqlViewGenerator::receivableMonthly();
        ...
        foreach ($resultSet as $item) {
            ...
            if (isset($data[$item['customer_id']]['values'][$yearMonth])) {
                $data[$item['customer_id']]['values'][$yearMonth] = doubleval($item['remaining']);
            }
    ...
    protected function saveToExcel($data, $records) {
        ...
        $documentProperties->setTitle('Laporan Piutang per Customer General');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Laporan Piutang per Customer');
        ...
        $worksheet->setCellValue('A2', 'Laporan Piutang Per Customer General');
        ...
        header('Content-Disposition: attachment;filename="Laporan Piutang Per Customer General.xls"');
