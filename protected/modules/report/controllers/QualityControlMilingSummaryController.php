...
class QualityControlMilingSummaryController extends Controller {
    ...
    public function filterAccess($filterChain) {
        if ($filterChain->action->id === 'summary') {
            if (!(Yii::app()->user->checkAccess('qcMilingReport')))
    ...
    public function actionSummary() {
        ...
        $qualityControlMilingSummary = new QualityControlMilingSummary($qualityControlMiling->search());
        $qualityControlMilingSummary->setupLoading();
        $qualityControlMilingSummary->setupPaging($pageSize, $currentPage);
        $qualityControlMilingSummary->setupSorting();
        $filters = array(
            'startDate' => $startDate,
            'endDate' => $endDate,
            'customerName' => $customerName,
        );
        $qualityControlMilingSummary->setupFilter($filters);
        if (isset($_POST['SaveToExcel'])) {
            $this->saveToExcel($qualityControlMilingSummary, $startDate, $endDate);
        }

        $this->render('summary', array(
            'qualityControlMiling' => $qualityControlMiling,
            'qualityControlMilingSummary' => $qualityControlMilingSummary,
            'startDate' => $startDate,
            'endDate' => $endDate,
    ...
    protected function saveToExcel($qualityControlMilingSummary, $startDate, $endDate) {
		set_time_limit(0);
		ini_set('memory_limit', '1024M');
        ...
        $documentProperties->setTitle('Laporan Quality Control Miling');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Laporan Quality Control Miling');
        ...
        $worksheet->setCellValue('A2', 'Laporan Quality Control Miling');
        ...
        foreach ($qualityControlMilingSummary->dataProvider->data as $header) {
            foreach ($header->qualityControlMilingDetails as $detail) {
                $worksheet->setCellValue("A{$counter}", $header->date);
                $worksheet->setCellValue("B{$counter}", CHtml::encode($header->getCodeNumber(QualityControlMilingHeader::CN_CONSTANT)));
                ...
                $worksheet->setCellValue("T{$counter}", CHtml::encode(CHtml::value($header, 'workOrderCuttingHeader.saleHeader.customer_order_number')));
                $worksheet->setCellValue("U{$counter}", empty($header->workOrderCuttingHeader->saleHeader->quotationHeader) ? "" : CHtml::encode($header->workOrderCuttingHeader->saleHeader->quotationHeader->getCodeNumber(QuotationHeader::CN_CONSTANT)));
        ...
        header('Content-Disposition: attachment;filename="Laporan Quality Control Miling.xls"');
