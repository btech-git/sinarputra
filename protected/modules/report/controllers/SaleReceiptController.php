...
class SaleReceiptController extends Controller {
    ...
    public function actionSummary() {
        ...
        $saleReceiptDetail = Search::bind(new SaleReceiptDetail('search'), isset($_GET['SaleReceiptDetail']) ? $_GET['SaleReceiptDetail'] : array());
        ...
        $saleReceiptDetailSummary = new SaleReceiptDetailSummary($saleReceiptDetail->search());
    ...
    protected function saveToExcel($saleReceiptDetailSummary, $startDate, $endDate) {
        ...
        $documentProperties->setTitle('Laporan Tanda Terima Penjualan');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Laporan Tanda Terima Penjualan');
        ...
        $worksheet->setCellValue('A2', 'Laporan Tanda Terima Penjualan');
        ...
        foreach ($saleReceiptDetailSummary->dataProvider->data as $header) {
            ...
            $number++;
            $saleReceiptHeader = $header->saleReceiptHeader;
            $saleInvoiceHeader = $header->saleInvoiceHeader;
            $worksheet->setCellValue("B{$counter}", $saleReceiptHeader->getCodeNumber(SaleReceiptHeader::CN_CONSTANT));
            ...
            $worksheet->setCellValue("H{$counter}", $saleInvoiceHeader->getCodeNumber(SaleInvoiceHeader::CN_CONSTANT));
        ...
        header('Content-Disposition: attachment;filename="Laporan Tanda Terima Penjualan.xls"');
