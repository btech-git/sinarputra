...
class ManualSalePaymentDetailController extends Controller {
    ...
    public function actionSummary() {
        ...
        $salePaymentHeader = Search::bind(new ManualSalePaymentHeader('search'), isset($_GET['ManualSalePaymentHeader']) ? $_GET['ManualSalePaymentHeader'] : array());
        ...
        $salePaymentSummary = new ManualSalePaymentSummary($salePaymentHeader->search());
    ...
    protected function saveToExcel($salePaymentSummary, $startDate, $endDate) {
        ...
        $documentProperties->setTitle('Laporan Pelunasan Detail Manual');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Laporan Pelunasan Detail Manual');
        ...
        $worksheet->setCellValue('A2', 'Laporan Pelunasan Detail Manual');
        ...
        foreach ($salePaymentSummary->dataProvider->data as $header) {
            $lastId = '';
            foreach ($header->manualSalePaymentDetails as $detail) {
                $saleReceiptDetail = ManualSaleReceiptDetail::model()->findByAttributes(array('manual_sale_invoice_header_id' => $detail->manual_sale_invoice_header_id));
                
                $worksheet->setCellValue("A{$counter}", CHtml::encode($header->date));
                $worksheet->setCellValue("B{$counter}", CHtml::encode($header->getCodeNumber(ManualSalePaymentHeader::CN_CONSTANT)));
                ...
                $worksheet->setCellValue("F{$counter}", CHtml::encode(CHtml::value($detail, 'manualSaleInvoiceHeader.employeeIdSalesman.name')));
                $worksheet->setCellValue("G{$counter}", CHtml::encode($detail->manualSaleInvoiceHeader->getCodeNumber(ManualSaleInvoiceHeader::CN_CONSTANT)));
                $worksheet->setCellValue("H{$counter}", CHtml::encode(CHtml::value($detail, 'manualSaleInvoiceHeader.tax_number')));
                $worksheet->setCellValue("I{$counter}", CHtml::encode($detail->manualSaleInvoiceHeader->date));
                $worksheet->setCellValue("J{$counter}", (empty($saleReceiptDetail->manualSaleReceiptHeader->date_receipt)) ? '' : CHtml::encode($saleReceiptDetail->manualSaleReceiptHeader->date_receipt));
                $worksheet->setCellValue("K{$counter}", CHtml::encode(CHtml::value($header, 'customer.invoice_due_days')));
                $worksheet->setCellValue("L{$counter}", (empty($saleReceiptDetail->manualSaleReceiptHeader->due_date)) ? '' : CHtml::encode($saleReceiptDetail->manualSaleReceiptHeader->due_date));
        ...
        header('Content-Disposition: attachment;filename="Laporan Pelunasan Detail Manual.xls"');
