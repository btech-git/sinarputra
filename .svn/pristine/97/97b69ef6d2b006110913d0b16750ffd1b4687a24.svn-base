...
class SaleReceiptDetailController extends Controller {
    ...
    public function actionSummary() {
        ...
        $saleReceiptSummary = new SaleReceiptSummary($saleInvoiceHeader->search());
    ...
    protected function saveToExcel($saleReceiptSummary, $startDate, $endDate) {
        ...
        $documentProperties->setTitle('Laporan Outstanding');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Laporan Outstanding');
        ...
        $worksheet->setCellValue('A2', 'Laporan Outstanding Per Customer Detail');
        ...
        foreach ($saleReceiptSummary->dataProvider->data as $header) {
            ...
            $worksheet->setCellValue("C{$counter}", empty($header->saleReceiptDetails) ? "" : CHtml::encode($header->saleReceiptDetails[0]->saleReceiptHeader->date_receipt));
            $worksheet->setCellValue("D{$counter}", CHtml::encode(CHtml::value($header, 'customer.invoice_due_days')));
            $worksheet->setCellValue("E{$counter}",  empty($header->saleReceiptDetails) ? "" : CHtml::encode(Yii::app()->dateFormatter->format('d MMMM yyyy',strtotime($header->saleReceiptDetails[0]->saleReceiptHeader->due_date))));
            $worksheet->setCellValue("F{$counter}", CHtml::encode($header->getCodeNumber(SaleInvoiceHeader::CN_CONSTANT)));
        ...
        header('Content-Disposition: attachment;filename="Laporan Outstanding Per Customer Detail.xls"');
