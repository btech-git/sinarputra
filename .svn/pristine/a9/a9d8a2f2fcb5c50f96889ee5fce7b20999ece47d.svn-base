...
class BloodTypeController extends CrudController {
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['BloodType'])) {
            $model->attributes = $_POST['BloodType'];
            if ($model->save())
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('BloodType');
    ...
    public function actionAdmin() {
        $model = new BloodType('search');
        $model->unsetAttributes();
        if (isset($_GET['BloodType']))
            $model->attributes = $_GET['BloodType'];
    ...
    public function loadModel($id) {
        $model = BloodType::model()->findByPk($id);
        if ($model === null)
