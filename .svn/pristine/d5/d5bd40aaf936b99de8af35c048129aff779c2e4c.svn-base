...
class SaleInvoiceDailyController extends Controller {
...
//            'customerId' => $customerId,
    ...
    protected function saveToExcel($saleInvoiceSummary, $startDate, $endDate) {
        ...
        $documentProperties->setTitle('Buku Penjualan');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Buku Penjualan');
        ...
        $worksheet->setCellValue('A2', 'Buku Penjualan : ' . $startDate . ' - ' . $endDate);
        ...
        foreach ($saleInvoiceSummary->dataProvider->data as $header) {
            foreach ($header->saleInvoiceDetails as $saleInvoiceDetail) {
                ...
                $worksheet->setCellValue("H{$counter}", CHtml::encode(CHtml::value($header, 'workOrderCuttingHeader.saleHeader.customer_order_number')));
                $worksheet->setCellValue("I{$counter}", CHtml::encode(CHtml::value($header, 'employeeIdSalesman.name')));
                $worksheet->setCellValue("J{$counter}", CHtml::encode($header->workOrderCuttingHeader->getCodeNumber(WorkOrderCuttingHeader::CN_CONSTANT)));

                if ($saleInvoiceDetail->workOrderCuttingDetail !== NULL) {
                    $worksheet->setCellValue("K{$counter}", CHtml::encode(CHtml::value($saleInvoiceDetail, 'workOrderCuttingDetail.product_name')));
                    $worksheet->setCellValue("L{$counter}", CHtml::encode(CHtml::value($saleInvoiceDetail, 'workOrderCuttingDetail.job_number')));
                    $worksheet->setCellValue("M{$counter}", CHtml::encode(CHtml::value($saleInvoiceDetail, 'workOrderCuttingDetail.productCategory.name')));
                    $worksheet->setCellValue("N{$counter}", CHtml::encode(CHtml::value($saleInvoiceDetail, 'workOrderCuttingDetail.height_request')));
                    $worksheet->setCellValue("O{$counter}", CHtml::encode(CHtml::value($saleInvoiceDetail, 'workOrderCuttingDetail.width_request')));
                    $worksheet->setCellValue("P{$counter}", CHtml::encode(CHtml::value($saleInvoiceDetail, 'workOrderCuttingDetail.length_request')));
                    $worksheet->setCellValue("Q{$counter}", CHtml::encode(CHtml::value($saleInvoiceDetail, 'workOrderCuttingDetail.height_quote')));
                    $worksheet->setCellValue("R{$counter}", CHtml::encode(CHtml::value($saleInvoiceDetail, 'workOrderCuttingDetail.width_quote')));
                    $worksheet->setCellValue("S{$counter}", CHtml::encode(CHtml::value($saleInvoiceDetail, 'workOrderCuttingDetail.length_quote')));
                }
        ...
        header('Content-Disposition: attachment;filename="Buku Penjualan.xls"');
