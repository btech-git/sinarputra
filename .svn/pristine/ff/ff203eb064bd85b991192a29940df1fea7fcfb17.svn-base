 ...
 */
class ReceivableLedgerBase extends ActiveRecord {

    public function tableName() {
        return 'tblsp_receivable_ledger';
    }

    public function rules() {
        return array(
            array('transaction_number, transaction_date, posting_datetime, admin_id, customer_id', 'required'),
            array('admin_id, customer_id', 'numerical', 'integerOnly' => true),
            array('transaction_number', 'length', 'max' => 60),
            array('debit, credit', 'length', 'max' => 18),
            array('note, memo', 'safe'),
            // The following rule is used by search().
            array('id, transaction_number, transaction_date, note, memo, debit, credit, posting_datetime, admin_id, customer_id', 'safe', 'on' => 'search'),
        );
    }
