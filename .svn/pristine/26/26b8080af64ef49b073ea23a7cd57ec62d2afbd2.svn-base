...
<div style="font-weight: bold; text-align: center">
    <div style="font-size: larger">Laporan C3 By Customer</div>
...
<table class="report">
        ...
        <td>
            <table>
                <tr>
                    ...
                    <th class="width2-3" style="text-align: right">Qty</th>
    ...
    $grandTotal = 0;
    ?>
    <?php foreach ($saleByCustomerSummary->dataProvider->data as $header): ?>
        <tr class="items1">
                        ...
                        <?php if ($saleHeader->date >= $startDate && $saleHeader->date <= $endDate): ?>
                            <tr>
                                <td class="width2-1" style="text-align: left;">
                                    <?php echo CHtml::encode(Yii::app()->dateFormatter->format('d MMM yyyy', CHtml::value($saleHeader, 'date'))); ?>
                                </td>
                                <td class="width2-2">
                                    <?php echo CHtml::encode($saleHeader->getCodeNumber(SaleHeader::CN_CONSTANT)); ?>
                                </td>
                                <td class="width2-3" style="text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', CHtml::value($saleHeader, 'totalQuantity'))); ?></td>
                                <td class="width2-4" style="text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', CHtml::value($saleHeader, 'grandTotalTransaction'))); ?>
