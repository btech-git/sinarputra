...
$this->breadcrumbs = array(
    'SPK Potong' => array('saleOrderList'),
    'View',
);
?>
...
<h1><?php echo 'SPK Potong /' . $this->action->id; ?></h1>
...
<?php $this->widget('zii.widgets.CDetailView', array(
    ...
    'attributes' => array(
        array(
            'label' => 'SPK Potong #',
            'value' => $model->getCodeNumber(WorkOrderCuttingHeader::CN_CONSTANT),
        ),
        array(
            'label' => 'Tanggal',
        ...
        array(
            'label' => 'SO #',
            'value' => $model->saleHeader->getCodeNumber(SaleHeader::CN_CONSTANT),
        ),
        array(
            'label' => 'Customer',
            'value' => $model->saleHeader->customer->company,
        ),
        array(
            'label' => 'Alamat Pusat',
            'value' => $model->saleHeader->customer->address_main,
        ),
        array(
            'label' => 'Alamat Kirim',
            'value' => $model->saleHeader->customer->address_secondary,
        ),
        array(
            'label' => 'Salesman',
            'value' => empty($model->saleHeader->employeeIdSalesman) ? "N/A" : $salesman->name,
        ),
        array(
            'label' => 'Customer PO #',
            'value' => $model->saleHeader->customer_order_number,
        ),
        array(
            'label' => 'Lembaran / Batangan',
            'value' => $model->saleHeader->originalMaterialStatus,
        ),
        array(
            'label' => 'Catatan',
            'value' => $model->note,
        ...
        array(
            'label' => 'Note',
            'value' => $model->saleHeader->customer->note,
        ),
        array(
            'label' => 'Pending',
            'value' => $model->pending,
...
<table style="border: 1px solid">
    <tr style="background-color: yellow">
        ...
        <th style="text-align: center;">Order Luar</th>
        <th style="width: 3%"></th>
    </tr>
    <tr>
        <td colspan= <?php echo ($model->is_service == 1) ? '18' : '17'; ?> >
            <table>
                <tr>
                    <th colspan="8" style="border-right: 1px solid; text-align: center">Material Awal</th>
                    <th colspan="10" style="border-left: 1px solid; text-align: center">REQUEST CUSTOMER</th>
                </tr>
    ...
    <?php foreach ($model->workOrderCuttingDetails as $cuttingDetail): ?>
        <tr style="background-color: yellow">
            ...
            <td style="text-align: center; border-top: 1px solid"><!--height-->
                <?php echo CHtml::encode(CHtml::value($cuttingDetail, 'height_quote')); ?>
            </td>

            <td style="text-align: center; border-top: 1px solid">
                <?php echo CHtml::encode(CHtml::value($cuttingDetail, 'width_quote')); ?>
            </td>

            <td style="text-align: center; border-top: 1px solid"><!--length awal-->
                <?php echo CHtml::encode(CHtml::value($cuttingDetail, 'length_quote')); ?>
            </td>

            <td style="text-align: center; border-top: 1px solid"><!--length awal-->
                <?php echo CHtml::encode(CHtml::value($cuttingDetail, 'quantity')); ?>
            </td>

            <td style="text-align: center; border-top: 1px solid"><!--weight-->
                <?php echo CHtml::encode(CHtml::value($cuttingDetail, 'weight')); ?>
            </td>
            ...
            <td style="text-align: center; border-top: 1px solid">
                <?php echo (CHtml::value($cuttingDetail, "is_annelying") == 1) ? "Yes" : ""; ?>
            </td>
            <td style="text-align: center; border-top: 1px solid">
            ...
            <?php endif; ?>
            <td style="text-align: center; border-top: 1px solid">
                <?php echo (CHtml::value($cuttingDetail, "is_urgent") == 1) ? "Yes" : ""; ?>
            </td>
            <td style="text-align: center; border-top: 1px solid">
                <?php echo (CHtml::value($cuttingDetail, "is_delivery") == 1) ? "Yes" : ""; ?>
            </td>
            <td style="text-align: center; border-top: 1px solid">
                <?php echo (CHtml::value($cuttingDetail, "is_external_order") == 1) ? "Yes" : ""; ?>
            </td>
            <td>
                <?php if (Yii::app()->user->checkAccess('workOrderEdit')): ?>
                    <?php echo CHtml::link("Update", array("update", "workOrderCuttingDetailId" => $cuttingDetail->id)); ?>
                <?php endif; ?>
            </td>
        </tr>
        <tr>
            <td colspan=<?php echo ($model->is_service == 1) ? '18' : '17'; ?>>
                <table>
                    <?php foreach ($cuttingDetail->workOrderCuttingDetailMaterials as $detail): ?>
                        <tr>
                            <?php $detailMaterial = ($detail->work_order_cutting_detail_material_id !== NULL) ? $detail->workOrderCuttingDetailMaterial : $detail->receiveDetail; ?>
                            <td style="width: 5%">
...
<table style="border: 1px solid">
    ...
    <?php foreach ($model->deliveryHeaders as $detail): ?>
        <tr>
            <td style="width: 15%">
                <?php echo CHtml::encode($detail->getCodeNumber(DeliveryHeader::CN_CONSTANT)); ?>
            </td>

            <td style="width: 15%"><!--name-->
                <?php echo CHtml::encode(Yii::app()->dateFormatter->format("d MMMM yyyy", CHtml::value($detail, 'date'))); ?>
            </td>

            <td>
                <?php echo CHtml::encode(CHtml::value($detail, 'customer_address')); ?>
            </td>

            <td style="text-align: center; width: 15%"><!--height-->
                <?php echo CHtml::encode(CHtml::value($detail, 'deliveryVehicle.plate_number')); ?>
            </td>

            <td style="text-align: center; width: 15%"><!--width order-->
...
<div id="link">
    ...
    <?php if ($flagCutting == 1): ?>
        <?php echo CHtml::link('Print SPK Potong', array('memo', 'id' => $model->id), array('target' => '_blank')); ?>
    <?php endif; ?>
    <?php if ((int) $flagMachining == 1) : ?>
        <?php echo CHtml::link('Print SPK Miling', array('memoMiling', 'id' => $model->id), array('target' => '_blank')); ?>
    <?php endif; ?>
</div>
