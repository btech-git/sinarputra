...
class SaleReceiptController extends Controller {
    ...
    public function actionDelete($id) {
        if (Yii::app()->request->isPostRequest) {
            ...
            if ($saleReceiptHeader !== null) {
                if ($saleReceiptHeader->header->salePaymentHeaders != NULL) {
    ...
    public function instantiate($id) {
        if (empty($id)) {
            $saleReceipt = new SaleReceiptComponent(new SaleReceiptHeader(), array());
        } else {
            $saleReceiptHeader = $this->loadModel($id);
            $saleReceipt = new SaleReceiptComponent($saleReceiptHeader, $saleReceiptHeader->saleReceiptDetails);
        }
