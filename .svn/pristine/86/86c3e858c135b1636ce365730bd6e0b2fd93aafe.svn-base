...
//            'access',
    ...
    public function filterAccess($filterChain) {
        if ($filterChain->action->id === 'summary') {
            if (!(Yii::app()->user->checkAccess('materialPaymentReport')))
    ...
    public function actionSummary() {
        ...
        $materialPaymentHeader = Search::bind(new MaterialPaymentHeader('search'), isset($_GET['MaterialPaymentHeader']) ? $_GET['MaterialPaymentHeader'] : array());
        ...
        $materialPaymentSummary = new MaterialPaymentSummary($materialPaymentHeader->search());
        $materialPaymentSummary->setupLoading();
        $materialPaymentSummary->setupPaging($pageSize, $currentPage);
        $materialPaymentSummary->setupSorting();
        $materialPaymentSummary->setupFilter($startDate, $endDate);

        if (isset($_POST['SaveToExcel'])) {
            $this->saveToExcel($materialPaymentSummary, $startDate, $endDate);
        }

        $this->render('summary', array(
            'materialPaymentHeader' => $materialPaymentHeader,
            'materialPaymentSummary' => $materialPaymentSummary,
            'startDate' => $startDate,
            'endDate' => $endDate,
    ...
    protected function saveToExcel($materialPaymentSummary, $startDate, $endDate) {
        set_time_limit(0);
        ini_set('memory_limit', '1024M');
        ...
        $documentProperties->setTitle('Laporan Pelunasan Manual');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Laporan Pelunasan Manual');
        ...
        $worksheet->setCellValue('A2', 'Laporan Pelunasan Manual');
        ...
        $worksheet->setCellValue('L6', 'Tgl TOP');
        ...
        foreach ($materialPaymentSummary->dataProvider->data as $header) {
            $lastId = '';
            foreach ($header->materialPaymentDetails as $detail) {
                
                $worksheet->setCellValue("A{$counter}", CHtml::encode($header->date_transaction));
                $worksheet->setCellValue("B{$counter}", CHtml::encode($header->getCodeNumber(MaterialPaymentHeader::CN_CONSTANT)));
                ...
                $worksheet->setCellValue("F{$counter}", CHtml::encode(CHtml::value($detail, 'materialInvoiceHeader.employeeIdSalesman.name')));
                $worksheet->setCellValue("G{$counter}", CHtml::encode($detail->materialInvoiceHeader->getCodeNumber(MaterialInvoiceHeader::CN_CONSTANT)));
                $worksheet->setCellValue("H{$counter}", CHtml::encode(CHtml::value($detail, 'materialInvoiceHeader.tax_number')));
                $worksheet->setCellValue("I{$counter}", CHtml::encode($detail->materialInvoiceHeader->date));
                $materialReceiptDetail = MaterialReceiptDetail::model()->findByAttributes(array('material_invoice_header_id' => $detail->material_invoice_header_id));
                $worksheet->setCellValue("J{$counter}", CHtml::encode(CHtml::value($materialReceiptDetail, 'materialReceiptHeader.date')));
                $worksheet->setCellValue("K{$counter}", CHtml::encode(CHtml::value($header, 'customer.invoice_due_days')));
                $worksheet->setCellValue("L{$counter}", CHtml::encode($detail->materialInvoiceHeader->due_date));
        ...
        header('Content-Disposition: attachment;filename="Laporan Pelunasan Manual.xls"');
