...
class ProductionCuttingHeader extends ProductionCuttingHeaderBase {

    const CN_CONSTANT = 'PRDC';
    ...
    public function searchForQualityControl() {
        ...
        $criteria->addCondition("EXISTS (
            SELECT production_cutting_header_id 
            FROM " . ProductionCuttingDetail::model()->tableName() . " 
            WHERE t.id = production_cutting_header_id AND quantity - quantity_quality_control > 0
