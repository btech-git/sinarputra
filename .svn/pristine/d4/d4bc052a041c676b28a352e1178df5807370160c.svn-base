...
class MaterialInvoice extends CComponent {
    ...
    public function generateCodeNumber($currentMonth, $currentYear) {
        $materialInvoiceHeader = MaterialInvoiceHeader::model()->find(array(
        ...
        if ($materialInvoiceHeader !== null) {
            $this->header->setCodeNumber($materialInvoiceHeader->cn_ordinal, $materialInvoiceHeader->cn_month, $materialInvoiceHeader->cn_year);
        }
    ...
    public function flush() {
        ...
        foreach ($this->details as $detail) {
            ...
            if ($detail->isNewRecord){
                $detail->material_invoice_header_id = $this->header->id;
            }
    ...
    public function getRemainingPayment() {
        return $this->getGrandTotal() - $this->header->total_payment;
    }
}